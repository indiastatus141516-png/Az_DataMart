...
router.get("/:userId", auth, requireRole("admin"), async (req, res) => {
  try {
  ...
  try {
    ...
    if (profile) {
      updateData.profile = {
        ...
        address: {
          street:
            profile.address?.street || existingUser.profile?.address?.street,
          ...
          zipCode:
            profile.address?.zipCode || existingUser.profile?.address?.zipCode,
          country:
            profile.address?.country || existingUser.profile?.address?.country,
        },
      };
    }
