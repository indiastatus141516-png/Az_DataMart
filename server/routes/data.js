      ...
      if (dayOfWeek && date) {
        try {
          ...
          if (!validDays.includes(dayOfWeek.toLowerCase())) {
            return res
              .status(400)
              .json({ message: "Invalid dayOfWeek. Use monday..friday" });
          }
          ...
          const requirement = await DailyRequirement.findOneAndUpdate(
            ...
            },
            {
              ...
              $set: {
                quantity:
                  (
                    await DailyRequirement.findOne({
                      category: categoryDoc.name,
                      dayOfWeek: dayOfWeek.toLowerCase(),
                      date: normalizedDate,
                    })
          ...
          try {
            if (
              ...
              const availableCount = await DataItem.countDocuments({
                category: categoryDoc.name,
                status: "available",
                "metadata.deliveryDate": normalizedDate
                  .toISOString()
                  .split("T")[0],
              });
          ...
          return res.json({
            message: `${jsonData.length} rows appended to daily requirement for ${categoryDoc.name} on ${dayOfWeek} ${date}`,
            requirement,
          });
        } catch (err) {
          ...
          return res
            .status(500)
            .json({ message: "Failed to append to daily requirement" });
        }
      }
...
router.get("/categories", auth, async (req, res) => {
  try {
    ...
    const fixedCategories = await require("../models/Category")
      .find()
      .sort({ id: 1 });

    const dynamicCategories = await DataItem.find({
      status: "available",
    }).distinct("category");
...
router.post("/import-sheets", requireAdmin, async (req, res) => {
  try {
    ...
    if (!spreadsheetId || !range) {
      return res
        .status(400)
        .json({ message: "Spreadsheet ID and range are required" });
    }
...
router.get("/daily-requirements", auth, async (req, res) => {
  try {
    ...
    requirementsData.forEach((r) => {
      ...
      if (!requirements[cat]) {
        requirements[cat] = {
          monday: { required: 0, uploaded: 0 },
          tuesday: { required: 0, uploaded: 0 },
          wednesday: { required: 0, uploaded: 0 },
          thursday: { required: 0, uploaded: 0 },
          friday: { required: 0, uploaded: 0 },
          total: 0,
        };
      }
...
router.get("/daily-data", auth, async (req, res) => {
  try {
    ...
    if (!category || !dayOfWeek || !date)
      return res
        .status(400)
        .json({ message: "category, dayOfWeek and date are required" });
    ...
    const uploadedData = items.map((it) => ({
      ...it.metadata,
      index: it.index,
    }));
