...
export const dataAPI = {
  ...
  getDailyRequirements: (params) =>
    api.get("/data/daily-requirements", { params }),
  getDailyUploadedData: (category, dayOfWeek, date) =>
    api.get("/data/daily-data", { params: { category, dayOfWeek, date } }),
...
const purchaseRequestUpdateInFlight = new Map();
...
export const adminAPI = {
  ...
  bulkDelete: (userIds) =>
    api.delete("/admin/users/bulk/delete", { data: { userIds } }),
  getPurchaseRequests: (params) =>
    api.get("/admin/purchase-requests", { params }),
  updatePurchaseRequest: (id, status) => {
    const key = `${id}:${status}`;
    const existing = purchaseRequestUpdateInFlight.get(key);
    if (existing) return existing;

    const reqPromise = api
      .put(`/admin/purchase-requests/${id}`, { status })
      .finally(() => {
        purchaseRequestUpdateInFlight.delete(key);
      });

    purchaseRequestUpdateInFlight.set(key, reqPromise);
    return reqPromise;
  },
  bulkDeletePurchaseRequests: (requestIds) =>
    api.delete("/admin/purchase-requests/bulk/delete", {
      data: { requestIds },
    }),
  ...
  getDataItemsByCategory: (category) =>
    api.get(`/admin/data-items/category/${category}`),
  updateDataItemPrice: (id, price) =>
    api.put(`/admin/data-items/${id}/price`, { price }),
  ...
  updateFixedCategory: (id, data) =>
    api.put(`/admin/categories/fixed/${id}`, data),
